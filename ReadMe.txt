========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : CompIdicChasen
========================================================================


AppWizard が生成した CompIdicChasen には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには CompIdicChasen を構成している各ファイルの概要説明が含まれています。
...
CompIdicChasen.h
    ...
    みます） をインクルードし、また CCompIdicChasenApp アプリケーションクラスを宣
    言します。

CompIdicChasen.cpp
    このファイルは CCompIdicChasenApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\CompIdicChasen.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル CompIdicChasen.rc によってインクルード
    されます。

res\CompIdicChasen.rc2
...
CompIdicChasenDlg.h, CompIdicChasenDlg.cpp - ダイアログ
    これらのファイルは CCompIdicChasenDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な CompIdicChasen.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) CompIdicChasen.pch 
